  ...
  return (
    ...
    initial={{opacity:0.2, y:100}} transition={{duration:1}} whileInView={{opacity: 1, y: 0}} viewport={{once:true}}>
        ...
        initial={{opacity:0, y:-20}} animate={{opacity: 1, y: 0}} transition={{delay:0.2,duration:0.8}}>
            <p>Best Text to Image Generate</p>
        ...
        className='flex flex-wrap justify-center mt-16 gap-3'>
            {Array(6).fill('').map((item,index)=>(
                ...
                className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={index%2===0 ? assets.sample_img_1 : assets.sample_img_2} alt="" keys={index} width={70}/>
            ))}
        </motion.div>
        <motion.p 
        initial={{opacity:0}}
        ...
        className='mt-2 text-neutral-600'>Generated images from Gen-Image</motion.p>
    </motion.div> 
  )
}
